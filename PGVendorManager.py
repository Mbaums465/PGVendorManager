...
class Vendor:
    def __init__(self, name, zone, council_left, last_reset, reset_maximum=0, categories=None):
        ...
        if isinstance(last_reset, str):
            ...
            # if it's a dict or unknown, assume now
...
def character_file_path(character_name):
    return os.path.join(DATA_DIR, f"{character_name}_vendors.json")
...
def save_vendors(vendors, character_name):
    ...
    # Save as list (compatible with older expectations)
    with open(file_path, 'w') as f:
        json.dump(vendor_list, f, indent=4)

def load_vendors(character_name):
    """
    Load vendors for character_name.
    Accept either:
      - a list of vendor dicts (old format),
      - or a dict {"vendors": [...], "character": ...} (alternate format).
    Return list[Vendor].
    """
    _ensure_data_dir()
    file_path = character_file_path(character_name)
    ...
    try:
        with open(file_path, 'r') as f:
            ...
            if isinstance(data, dict):
                ...
                if isinstance(vendors_blob, list) and vendors_blob:
                    return [Vendor.from_dict(v) for v in vendors_blob]
                # maybe the dict itself is the vendor entry
                # as a fallback try to read any dict-like vendor list fields
...
class VendorApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        #Button(btns, text="Refresh", command=self.update_vendor_list).pack(side=tk.LEFT, padx=4)#who cares? I removed this.,
    ...
    def add_new_character(self):
        ...
        if os.path.exists(default_path):
            ...
            save_vendors(default_vendors, name)
        ...
        for widget in self.scrollable_frame.winfo_children():
            if hasattr(widget, 'vendor_name'):
                ...
                if vendor:
                    ...
                    # store time_label as attribute when creating entries below
                    if getattr(widget, "time_label", None):
    ...
    def open_add_vendor_window(self):
        ...
        # override on the left (aligned with categories)
        ...
        def add_and_save():
            ...
            new_vendor = Vendor(name, zone, council, last_reset.isoformat(), reset_maximum, final_cats)
            # Convert last_reset string to datetime in constructor
            # (Vendor.from dict handling)
            # However above we passed last_reset as iso string; constructor will parse it.
...
if __name__ == "__main__":
    ...
    def on_close():
        save_vendors(app.vendors, app.current_character)
        app.destroy()
